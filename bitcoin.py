...
class Bitcoin(miner.Miner):
    ...
    def addToChain(self, tAdd, sender):
        ...
        for n in temp:
            t = n.tx
            # only ever one pointer in Bitcoin
            parent = self.findInChain(t.pointers[0])
            if first:
                # do not broadcast if oldest parent is orphan (includes orphan AND children of orphans!!)
                broadcast = not self.childOfOrphan(t)
                # putting this here means that orphans are marked PRE before they join the genesis-rooted tree
        ...
        if maxDepth > 0:
            if mx == maxDepth and mx - d >= self.o.bitcoinAcceptDepth:
                ...
                if node.tx.id in self.reissue:
                    # accepted, so it doesn't need to be reiussed (this will happen because reissued tx are still saved in old forks)
                ...
                if node.tx in self.sheep and mx < maxDepth - self.o.bitcoinAcceptDepth:
                    # the order these will be added to reissue is leaf-up
                    self.reissue.add(node.tx.id)
        return mx
